...
$level_id=@$rank_level[1]->level_id;
...
$nxt_level_rank=$this->common_model->common($table_name = 'tbl_user_rank', $field = array(), $where = array('set_id'=>$set_id,'level_id'=>$level_id), $where_or = array(), $like = array(), $like_or = array(), $order = array(), $start = '', $end = '');

if(count($nxt_level_rank)>0){
...
for ($i=0; $i <count($enroll_details) ; $i++) { 
  if($enroll_details[$i]->user_id==$active_usr){
    $j++;
  }
}
}
...
else if(count($user_exam)<=0){ echo "Over"; }
 

else if(@$j==1 && count($nxt_level_cre)<=0){ 

  ?> 
 


 <a href="<?php echo $this->url->link(124).'/'.$set_dtls[0]->set_slug;?>" class="btn btn-success">Go to Next Level</a> <?php }else{ echo "Over"; }  ?> </td>
