...
<body  style="height: 100%; width:100%; position: fixed; resize:none; "  >
<?php
 ...
 $exam_name=$this->common_model->common($table_name = 'tbl_exam_type', $field = array(), $where = array('id'=>@$exam), $where_or = array(), $like = array(), $like_or = array(), $order = array(), $start = '', $end = ''); ?>
    <header class="testpageheader">
        <div class="wrapper">
            <div class="inrhdr">
              <span class="logo"><img src="<?php echo base_url().'assets/images/site-logo.png';?>"></span> 
                <span class="time-left">
                    <ul>
                        <li><strong>Full Marks:</strong> <?php echo round(@$set_dtls[0]->total_marks); ?></li>
                        <li><strong>Subject:</strong> <?php echo @$subject_name[0]->section_name; ?></li>
                        <?php if(@$set_dtls[0]->test_type==5 || @$set_dtls[0]->test_type==6){ ?>
                           <li><strong>Chapter:</strong> <?php echo @$chapter_name[0]->chap_name; ?></li> <?php } ?>
                 <li><strong>Set Code:</strong> <?php echo $set_dtls[0]->set_code; ?></li>
                <li><strong>Duration:</strong> <?php echo @$dur;?> Secs</li>
               <li> <strong> Remaining Time in secs:</strong> <span   id="timer"> <?php echo @$dur;?></span >        </li> 
                    </ul>    
                </span> 
               ...
               <!-- <div class="ex-title text-center">
                ...
                <span class="profile">
                    <span class="profileimg"><img src="<?php if(@$user_details[0]->profile_photo!=''){echo base_url().'assets/uploads/profile_image/'.@$user_details[0]->profile_photo; }else{echo base_url().'assets/images/profile.jpg';}?>"></span>
                    <span class="stdntnam"><?php echo @$user_details[0]->user_name; ?></span>
                    <span><?php echo date('jS M Y',strtotime(@$set_dtls[0]->exam_date)); ?> <?php echo @$set_dtls[0]->exam_time;
                                                                $time_arr=explode(':',@$set_dtls[0]->exam_time);if($time_arr[0]>=12){echo ' PM';}else{echo ' AM'; } ?></span> 
...
<span class="exam-bar" style="font-size:30px;cursor:pointer" onclick="openNav()"><i class="fa fa-bars" aria-hidden="true"></i></span>
    ...
    <div class="nano-content">
        ...
        $exm_qstn=$this->join_model->get_question_of_section_knockouttest($set_id[0],$pattern[0]->subject_id,$level_id);
        ?>
...
<div class="footrlft">
    <a class="arrow_slide2" id="save" style="display: block" href="javascript:void(0);" onclick="save_next_question_for_knockout('<?php echo @$exam; ?>','<?php echo @$set_id; ?>','<?php echo @$qstn[0]->section_id;?>','<?php echo  @$q_id; ?>','<?php echo @$level_id; ?>')">SAVE & NEXT &nbsp;<span class="angle2"></span></a>
...
<div class="rightpart" id="rightpart">
  <div id="about" class="nano rightmain">
    <div class="nano-content">
        ...
        $exm_qstn=$this->join_model->get_question_of_section_knockouttest($set_id,$pattern[0]->subject_id,$level_id);
        ...
        <h3 class="acrdnhdr"><?php echo @$subject_name[0]->section_name; ?> ( <?php echo @$exam_name[0]->exam_name; ?> )</h3>
            <div>
                <div style="display:inline-block;width:100%">
                    ...
                    ?>
                        <a href="javascript:void(0)" onclick="get_knockout_single_question('<?php echo @$qs_id; ?>','<?php echo @$subject_id; ?>','<?php echo $i;?>','<?php echo $set_id;?>','<?php echo $level_id;?>')"><li class="questionnumb unattempted" id="q_<?php echo @$qs_id; ?>"><?php echo $i;?></li></a>
...
<div class="submit  res-768-disnone">
    ...
    <a type="button" id="com_sub_id" class="submitbtn" onclick="submit_examination()">Submit test</a>
</div>
</div>
</div>
...
<script>
function  get_knockout_single_question(question_id,section_id,question_no,set_id,level_id)
{
    ...
    var prev_se=$("#hid_se").val();
     ...
     var base_url='<?php echo base_url(); ?>';
                ...
                url:'<?php echo base_url(); ?>manage_exam/get_knockout_single_question',
                data:{q_id:question_id,q_no:question_no,s_id:set_id,level_id:level_id},
                async: false,
                ...
                success: function (data) {
                            ...
                            var node='save_next_question_for_knockout('+e_id+','+set_id+','+se_id+','+q_id+','+level_id+')';
                             // alert(q_id); 
...
//alert(second);
 ...
 if(seconds == 1) {
      document.getElementById('timer').innerHTML="all done, bye bye";
...
$('#com_sub_id').attr('disabled', true);
                        ...
                        url:'<?php echo base_url(); ?>manage_exam/complete_examination',
                        data:{sec:seconds,setid:set,exmid:exam_id,endtime:endtm,remaining_time:remaining_time},
                        async: false,
...
<script>
function save_next_question_for_knockout(exam_id,set_id,section_id,question_id,level_id)
{
    // alert(question_id); 
    ...
    var sec_array = sections.split(',');
     var base_url='<?php echo base_url(); ?>';
               ...
               dataType: 'html',
                ...
                url:'<?php echo base_url(); ?>manage_exam/save_next_question_for_knockout',
                data:{e_id:exam_id, s_id:set_id, se_id:section_id, q_id:question_id,rvw:
                    review,answ:answer_no,level_id:level_id},
                async: false,
...
document.getElementById("save").style.display = "none";
                            ...
                            var q_id=$("#hid_q").val();
                              var level_id=$("#level_id").val();
                            // alert(q_id);
                            var node='save_next_question_for_knockout('+e_id+','+s_id+','+se_id+','+q_id+','+level_id+')';
                            $('#save').attr('onclick', node);
