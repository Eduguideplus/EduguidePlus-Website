...
if(count($passage_ids)>0)
{
    ...
    function get_exam_question_details($user_id,$s_id,$q_id,$exam_id)
    {
         $this->db->select('*');
         $this->db->from('tbl_user_exam tue');
         $this->db->join('tbl_user_exam_details tued','tue.id=tued.user_exam_id');
         $this->db->where('tue.user_id',$user_id);
         $this->db->where('tue.set_id',$s_id);
         $this->db->where('tued.question_master_id',$q_id);
         $this->db->where('tued.user_exam_id',$exam_id);
         $query=$this->db->get();
         return $query->result();
