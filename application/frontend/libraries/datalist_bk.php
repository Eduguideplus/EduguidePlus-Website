...
class datalist {
	...
	function supplierlist(){
		$CI =& get_instance();
		$CI->load->model('admin_model');
		$data['supplierlist']=$CI->admin_model->selectOne('tbluser_business','userb_role',2);
		$data['supplier']='';
		foreach($data['supplierlist'] as $row_supplier)
		{
			$data['supplier'].='<option value="'.$row_supplier->userb_id.'">'.$row_supplier->userb_company.'</option>';
		}
	...
	function edit_product_equivalent($id)
	{
		...
		$company_list = $this->admin_model->selectAll('tblcompany');		
		$js_company_list ='[';
			$tag_count=0;			
		...
		$str.='<script language="javascript" type="text/javascript">$(document).ready(function(){';
		$str.=' jQuery(".list_equ_brand").tagsManager({ prefilled: [], CapitalizeFirstLetter: true, preventSubmitOnEnter: true,typeahead: true,';
		$str.='  typeaheadAjaxSource: null,typeaheadSource:'.$data['js_brand_list'].',';
		$str.=' delimeters: [44, 188, 13],   backspace: [8],';  
		$str.=" blinkBGColor_1: '#FFFF9C', blinkBGColor_2: '#CDE69C',hiddenTagListName: 'hiddenTagListA'";
		$str.='  });'; 
