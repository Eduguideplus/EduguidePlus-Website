...
public function download_csv()
{
  ...
  header("Content-Disposition: attachment; filename=".$filenae.".csv");
          ...
          fputcsv($output, array('SL No.','Candidate Name','Batch','Date & Time','Test Type','Chapter','Total Questions','Time Allotted','Question Attempted','Right Answer','Score','%'));
         ...
         $get_userdetail= $this->common_model->common($table_name = 'tbl_user', $field = array(), $where = array('id'=>$res->user_id), $where_or = array(), $like = array(), $like_or = array(), $order = array(), $start = '', $end = '');
          ...
          $get_batch_details= $this->common_model->common($table_name = 'tbl_batch', $field = array(), $where = array('batch_id'=>$get_userdetail[0]->batch_id), $where_or = array(), $like = array(), $like_or = array(), $order = array(), $start = '', $end = '');
  ...
  fputcsv($output, array(
        ...
        "Test Type"=>$res->test_select_type ,
        "Chapter"=>$chapterName,
        'Total Questions' =>round($res->q_qty),
        'Time Allotted' =>gmdate('H:i:s',$res->exam_duration ),
        'Question Attempted'=>round($res->attempt_count),
        'Right Answer' =>round($res->obtained_marks),
        'Score' =>round($res->obtained_marks),
        '%'=>$percentage
