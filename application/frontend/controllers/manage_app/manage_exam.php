   ...
   /*     $passage_question=$this->join_model->get_knowledge_set_passage_question($set);
	...
	public function start_examination()
	{
		...
		$set_id=$obj->set_id;
			...
			$qstn=$this->join_model->get_question_of_section_knowledgetest($set_id,$subject_id);
		        ...
		        $set_question=$this->join_model->get_knowledge_set_question($set_id);
				 ...
				 $check_user_exam= $this->common_model->common($table_name = 'tbl_user_exam', $field = array(), $where = array('user_id'=>$user_id,'set_id'=>$set_id), $where_or = array(), $like = array(), $like_or = array(), $order = array(), $start = '', $end = '');
					...
					$current_question=$qstn[0]->question;
					$current_question_marks=$qstn[0]->marks;
					$current_question_neg_marks=$qstn[0]->neg_mark; 
					$q_id= $qstn[0]->id;
					...
					$current_ans_array=array();
					$sl=0;

					foreach($answr as $res)
					{
							$sl++;
							$ansr_id= $res->id;


							$check_answer_array= $this->common_model->common($table_name = 'tbl_user_exam_details', $field = array(), $where = array('selected_choice'=>$ansr_id,'user_exam_id'=>$user_exam), $where_or = array(), $like = array(), $like_or = array(), $order = array(), $start = '', $end = '');

							if(count($check_answer_array)>0){
								$isattempted='y';
							...
							$resarray['answer_id']= $res->id;
							$resarray['choice']= $res->choice;
							$resarray['sl']= $sl;
							$resarray['isattempted']= $isattempted;


							array_push($current_ans_array, $resarray);
					}
$total_question_array=array();
$qno=0;
$no_of_quest=count($qstn);
foreach($qstn as $row)
{
		$qno++;
		$questionId= $row->id;

		$answr_list=$this->common_model->common($table_name = 'tbl_question_choice', $field = array(), $where = array('que_id'=>$questionId), $where_or = array(), $like = array(), $like_or = array(), $order = array(), $start = '', $end = '');

		$check_exam_details_array= $this->common_model->common($table_name = 'tbl_user_exam_details', $field = array(), $where = array('question_master_id'=>$questionId,'user_exam_id'=>$user_exam), $where_or = array(), $like = array(), $like_or = array(), $order = array(), $start = '', $end = '');
		if(count($check_exam_details_array)>0)
		{
			$attempted='y';
		}
		else{
		...
		$answerArray=array();
		$ansno=0;
		foreach($answr_list as $ans)
		{
			$ansno++;

			$ansr_id= $ans->id;

			$check_answer_array_list= $this->common_model->common($table_name = 'tbl_user_exam_details', $field = array(), $where = array('selected_choice'=>$ansr_id,'user_exam_id'=>$user_exam), $where_or = array(), $like = array(), $like_or = array(), $order = array(), $start = '', $end = '');

				if(count($check_answer_array_list)>0){
								$isattempted='y';
			...
			$ansarray['answer_id']= $ans->id;
			$ansarray['choice']= $ans->choice;
			$ansarray['sl']= $ansno;
			$ansarray['isattempted']=$isattempted;

			array_push($answerArray, $ansarray);

		}
		$rowarray['questionId']=$row->id;
		$rowarray['question']=$row->question;
		$rowarray['marks']=$row->marks;
		$rowarray['neg_mark']=$row->neg_mark;
		$rowarray['answerArray']=$answerArray;
		$rowarray['qno']=$qno;
		$rowarray['attempted']=$attempted;

		array_push($total_question_array, $rowarray);
...
echo json_encode(array('user_exam_id'=>$user_exam,'current_question'=>$current_question,'current_question_marks'=>$current_question_marks,'current_question_neg_marks'=>$current_question_neg_marks,'q_id'=>$q_id,'current_ans_array'=>$current_ans_array,'current_question_no'=>$current_question_no,'total_question_array'=>$total_question_array,'no_of_quest'=>$no_of_quest,'subject_id'=>$subject_id,'set_id'=>$set_id));
	}
...
<input type="hidden" name="hid_q" id="hid_q" value="<?php echo @$q_id;?>">
	...
	public function get_single_question()
	{
		...
		$user_exam_details=$this->common_model->common($table_name = 'tbl_user_exam', $field = array(), $where = array('user_id'=>@$user_id,'set_id'=>$s_id), $where_or = array(), $like = array(), $like_or = array(), $order = array('id'=>'DESC'), $start = '', $end = '');
		$user_examId= $user_exam_details[0]->id;

		 $question_prev_status=$this->join_model->get_exam_question_details($user_id,$s_id,$q_id,$user_examId);
...
public function save_next_question_app()
{
	$json =  file_get_contents('php://input');
    $obj =  json_decode($json);
    ...
    $user_id= $obj->user_id;
    $q_id= $obj->q_id;
    $userexam_id= $obj->userexam_id;
    $attempt_ans_id= @$obj->attempt_ans_id;
    $is_correct='No';
    $current_date= date("Y-m-d H:i:s"); 

    if($attempt_ans_id>0)
    {
    		$get_quest_right_ans=$this->common_model->common($table_name = 'tbl_question_choice', $field = array(), $where = array('que_id'=>$q_id,'is_correct'=>'Yes'), $where_or = array(), $like = array(), $like_or = array(), $order = array(), $start = '', $end = '');

    		$corect_ans_id=$get_quest_right_ans[0]->id;

    		if($attempt_ans_id==$corect_ans_id)
    		{
    			$is_correct='Yes';
    		} 
    		else{
    ...
    $check_exam_record=$this->common_model->common($table_name = 'tbl_user_exam_details', $field = array(), $where = array('user_exam_id'=>$userexam_id,'question_master_id'=>$q_id), $where_or = array(), $like = array(), $like_or = array(), $order = array(), $start = '', $end = '');

    if(count($check_exam_record)>0)
			{
				$data_array=array(
				
				'selected_choice'=>$attempt_ans_id,
				'correct_choice'=>$corect_ans_id,
				'is_correct'=>$is_correct,
				'is_review'=>'No',
				'created_on'=>$current_date
				);
				$this->db->where('user_exam_id',$userexam_id);
...
foreach($qstn as $row)
{
		...
		$check_exam_details_array= $this->common_model->common($table_name = 'tbl_user_exam_details', $field = array(), $where = array('question_master_id'=>$questionId,'user_exam_id'=>$userexam_id), $where_or = array(), $like = array(), $like_or = array(), $order = array(), $start = '', $end = '');
		if(count($check_exam_details_array)>0)
		{
			$attempted='y';
		}
		...
		foreach($answr_list as $ans)
		{
			...
			$check_exam_ans_array= $this->common_model->common($table_name = 'tbl_user_exam_details', $field = array(), $where = array('selected_choice'=>$ansId,'user_exam_id'=>$userexam_id), $where_or = array(), $like = array(), $like_or = array(), $order = array(), $start = '', $end = '');

			if(count($check_exam_ans_array)>0)
			{
					$isattempted='y';
			}
			else{
    ...
    echo json_encode(array('result'=>$result,'total_question_array'=>$total_question_array));
}
...
// print_r($nex_answr);
								...
								$question_prev_status=$this->join_model->get_exam_question_details($user_id,$s_id,$next_q_id,$user_exam_id);
...
$this->session->unset_userdata('activeuser');
	...
	public function submit_test_app()
	{
		$json =  file_get_contents('php://input');
    	$obj =  json_decode($json);

    	$user_id=$obj->user_id;
		$exmid=$obj->userexam_id;
		$set=$obj->set_id;
		$endtime=date('Y-m-d H:i:s'); 
		$remaining_time=@$obj->end_time;
		...
		{
			$q_id=$res->question_master_id;

			$qstn_dtls=$this->common_model->common($table_name = 'tbl_questions', $field = array(), $where = array('id'=>$q_id), $where_or = array(), $like = array(), $like_or = array(), $order = array(), $start = '', $end = '');

			$marks= $qstn_dtls[0]->marks;
			$neg_marks_each=$qstn_dtls[0]->neg_mark;

			if($res->is_correct=='Yes')
			{
			...
			if($res->is_correct=='No')
			{
				
				$ob_marks=$ob_marks-$neg_marks_each;
	...
	public function get_test_results()
	{
		...
		$user_exam_details=$this->common_model->common($table_name = 'tbl_user_exam', $field = array(), $where = array('id'=>$user_exam_id), $where_or = array(), $like = array(), $like_or = array(), $order = array(), $start = '', $end = '');


		 $set_dtls=$this->common_model->common($table_name = 'tbl_knowledge_qiuz_set', $field = array(), $where = array('kq_id'=>$set_id), $where_or = array(), $like = array(), $like_or = array(), $order = array(), $start = '', $end = '');

		 $test_type= $this->common_model->common($table_name = 'tbl_test_type', $field = array(), $where = array('test_id'=>$set_dtls[0]->test_type), $where_or = array(), $like = array(), $like_or = array(), $order = array(), $start = '', $end = '');



		$exam_type= $this->common_model->common($table_name = 'tbl_exam_type', $field = array(), $where = array('id'=>$set_dtls[0]->exam_id), $where_or = array(), $like = array(), $like_or = array(), $order = array(), $start = '', $end = '');

		$subject= $this->common_model->common($table_name = 'tbl_question_section', $field = array(), $where = array('id'=>$set_dtls[0]->subject_id), $where_or = array(), $like = array(), $like_or = array(), $order = array(), $start = '', $end = '');

		$total_queston= $this->common_model->common($table_name = 'tbl_knowledge_quiz_set_dtls', $field = array(), $where = array('set_id'=>$set_id), $where_or = array(), $like = array(), $like_or = array(), $order = array(), $start = '', $end = '');

		$testname= @$set_dtls[0]->set_name;
		$examtypename= @$exam_type[0]->exam_name;
		$subjectname= @$subject[0]->section_name;
		$total_question= count($total_queston);
		$attempted= count($answer_list);
		$unattempted= intval($total_question)-intval($attempted);
		$correct_answer= $user_exam_details[0]->curr_ans;
		$wrong_answer= $user_exam_details[0]->wrong_ans;
		$obtained_marks= $user_exam_details[0]->obtained_marks;
		$exam_duration= @$set_dtls[0]->exam_duration;
		$total_marks= @$set_dtls[0]->total_marks;

		echo json_encode(array('testname'=>$testname,'examtypename'=>$examtypename,'subjectname'=>$subjectname,'total_question'=>$total_question,'attempted'=>$attempted,'unattempted'=>$unattempted,'correct_answer'=>$correct_answer,'wrong_answer'=>$wrong_answer,'obtained_marks'=>$obtained_marks,'exam_duration'=>$exam_duration,'total_marks'=>$total_marks));
	}
...
$notify_attemp=round($no_of_question*0.75);
		...
		//echo '<pre>'; print_r($answer_list); exit;
...
$level_id=@$rank_level[0]->level_id;
		...
		for($i=0;$i<count($answer_list);$i++)
		{
			...
			if($answer_list[$i]->is_correct=='No')
			{
				$mks_each_question=@$qstn_dtls[0]->marks;
				$neg_marks_each=@$qstn_dtls[0]->neg_mark;
				//$got_marks= $mks_each_question-$neg_marks_each;
				$ob_marks=$ob_marks-$neg_marks_each;
...
$first=intval($nca)/intval($tnq);
$second=intval($ts)/intval($tt);
$third=intval($nwa)/intval($tnq);
$fourth=intval($tnq)/intval($tt);

$step1=intval($first)+intval($second);
$step2=intval($third)*intval($fourth);
$step3=intval($step2)*intval($nwa);

$step4=intval($step1)-intval($step3);
$step5=intval($step4)-intval($nmwa);
$step6=intval($step5)/intval($nc);

// $rank_index=$nca+((($nca/$tnq)+($ts/$tt)-($nwa/$tnq)*($tnq/$tt)*$nwa-($nmwa))/$nc);
$rank_index=intval($nca)+intval($step6);
