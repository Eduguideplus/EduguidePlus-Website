...
if($test_type==4){
  ...
  {     
...

	{
...

		{
...

			{
					...
					$all_anwer_array= $this->common_model->common($table_name = 'tbl_question_choice', $field = array(), $where = array('que_id'=>$quest_id), $where_or = array(), $like = array(), $like_or = array(), $order = array(), $start = '', $end = '');
					...
					foreach($all_anwer_array as $res)

					{

						$c++;

						$rowanswer['optionno']= $c;

						$rowanswer['option']= $res->choice;	



						array_push($option_array, $rowanswer);
					...
					$rowarray['option_array']=$option_array;
...

	{
		...
		$set_slug=$this->uri->segment(3);

		if($set_id!='')
		{

			$get_set_details= $this->common_model->common($table_name = 'tbl_knowledge_qiuz_set', $field = array(), $where = array('kq_id'=>$set_id), $where_or = array(), $like = array(), $like_or = array(), $order = array(), $start = '', $end = '');


			if(count($get_set_details)>0)
			{
				if($this->session->userdata('activeuser')!='')
...

				{
					...
					$this->session->set_userdata('come_from','demo_join');
				...
				$test_type_id=$get_set_details[0]->test_type;
		...
		$user_knowledge=$this->common_model->common($table_name = 'tbl_user_knowledge_quiz', $field = array(), $where = array('set_id'=>$set_id,'user_id'=>$user_id), $where_or = array(), $like = array(), $like_or = array(), $order = array(), $start = '', $end = '');
		 ...
		 $data_arr=array(
						...
						'test_select_type'=>'Subject Wise Test',
...
echo json_encode(array('testname'=>$testname,'examtypename'=>$examtypename,'subjectname'=>$subjectname,'total_question'=>$total_question,'exam_duration'=>$exam_duration,'total_marks'=>$total_marks));
