     ...
     $objPHPExcel=$objReader->load('../assets/uploads/excel/'.$file_name);    
         ...
         $current_date=date('Y-m-d H:i:s');
          ...
          for($i=2;$i<=$totalrows;$i++)
          {
              ...
              $time=$objWorksheet->getCellByColumnAndRow(15,$i)->getValue();
               $marks=$objWorksheet->getCellByColumnAndRow(16,$i)->getValue();
                $neg_marks=$objWorksheet->getCellByColumnAndRow(17,$i)->getValue();
              ...
              if($question!='' && $subject!='' && $group_id!='' && $exam!='' && $chapter!='' && $time!='' && $marks!='' && $neg_marks!='')
              {
                  $data_user=array(
                                      'question'=>$question,
                                      'section_id'=>$subject,
