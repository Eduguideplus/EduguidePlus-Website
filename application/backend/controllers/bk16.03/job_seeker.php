...
function job_seeker_list_view()
	{
	  ...
	  // $data['career_details']=$this->admin_model->order_date('tbl_career');
        //$data['career_details']= $this->common->select($table_name='tbl_career',$field=array(),
          ...
          $data['job_seeker_details']=$this->common_model->common($table_name ='tbl_job_seeker', $field = array(), $where = array(), $where_or = array(), $like = array(), $like_or = array(), $order = array(), $start = '', $end = '');
		 ...
		 $data['active']='seeker';
		  ...
		  $this->load->view('job_seeker/job_seeker_list',$data);
...
function delete_career()
	{
		...
		$id= $this->uri->segment(3);
		//echo $id;
		$career_details= $this->common->select($table_name='tbl_job_seeker',$field=array(),
			$where=array('job_seeker_id'=>$id),$like=array(),$order=array(),$where_or=array(),$like_or_array=array());
		$image = $career_details[0]->candidate_resume;
		//echo $image; exit;
		//$count = count($get_details);
		//echo $count; exit;
		@unlink("../assets/uploads/career_cv/".@$image);

		$this->db->where('job_seeker_id',$id);
		$this->db->delete('tbl_job_seeker');
		$this->session->set_flashdata('succ_delete','One job has been deleted');
		redirect('index.php/job_seeker/job_seeker_list_view');
	}
	...
	function change_to_active()
  {
		...
		for($i=0;$i<count($test_ids);$i++)
		{
			...
			if($this->db->update('tbl_job_seeker',$data))
			{
				$result=1;
			}
