...
public function list_view()
{
	...
	$data['principal_list'] = $this->common_model->common($table_name = 'tbl_user', $field = array(), $where = array('user_type_id ='=>'3'), $where_or = array(), $like = array(), $like_or = array(), $order = array('id'=>'DESC'), $start = '', $end = '');
    $data['active']='principal';
	$this->load->view('template/admin_header');
	$this->load->view('template/admin_leftmenu',$data);
	$this->load->view('manage_principal/principal_table',$data);
...
function add_principal()
{
    ...
    $this->load->view('template/admin_header');
    $this->load->view('template/admin_leftmenu',$data);
    $this->load->view('manage_principal/principal_add',$data);
    $this->load->view('template/adminfooter_category');
}
function insert()
{
      ...
      $full_name=$this->input->post('fname');   
    	...
    	if(($_FILES['pro_pic']['name'])!='')
		{
            ...
            {
                move_uploaded_file($file_tmp, "../assets/uploads/principal/" . $new_name . "." . $ext);
                
                $image = $new_name . "." . $ext;
                
                $img_config_4['image_library'] = 'gd2';
                $img_config_4['source_image'] = '../assets/uploads/principal/' . $image;
                $img_config_4['create_thumb'] = FALSE;
                $img_config_4['maintain_ratio'] = FALSE;
                $img_config_4['width']  = 150;
                $img_config_4['height'] = 150;
                $img_config_4['new_image'] ='../assets/uploads/principal/' . $image;
        ...
        $type_id_details=$this->common_model->common($table_name = 'tbl_user', $field = array(), $where = array('user_type_id'=>'3'), $where_or = array(), $like = array(), $like_or = array(), $order = array(), $start = '', $end = '');
        ...
        $final_user_code='PRIN'.$in_id;
    	$data_code = array('user_code' => $final_user_code);
        // print_r($final_user_code); exit;
    	$this->db->where('id', $insert_id);
  		$this->db->update('tbl_user', $data_code);

    	$this->session->set_flashdata('succ_msg','One Principal added successfully');
    	redirect('index.php/manage_principal/list_view/','refresh');
}
...
function edit_view()
{
    ...
    $principal_id =$this->uri->segment(3);
    
    $data['principal_details'] = $this->common_model->common($table_name = 'tbl_user', $field = array(), $where = array('id'=>$principal_id,'user_type_id'=>'3'), $where_or = array(), $like = array(), $like_or = array(), $order = array(), $start = '', $end = '');
    ...
    $country_id= @$data['principal_details'][0]->country;
	...
	$state_id= @$data['principal_details'][0]->state;
    ...
    $this->load->view('template/admin_header');
    $this->load->view('template/admin_leftmenu',$data);
    $this->load->view('manage_principal/principal_edit',$data);
    $this->load->view('template/adminfooter_category');
 }
      ...
      $full_name=$this->input->post('fname');   
		...
		else
		{
            ...
            if ($ext == "png" || $ext == "jpg" || $ext == "jpeg" || $ext == "gif")
            {
            	if(@$hidden_image!='')
            	{
            		unlink('../assets/uploads/principal/'.@$hidden_image);
            	}
                move_uploaded_file($file_tmp, "../assets/uploads/principal/" . $new_name . "." . $ext);
    	...
    	$this->session->set_flashdata('succ_msg','One Principal updated successfully');
    	redirect('index.php/manage_principal/list_view/','refresh');
}
...
public function details_view()
{
	...
	$principal_id = $this->uri->segment(3);
	$data['user_details'] = $this->common_model->common($table_name = 'tbl_user', $field = array(), $where = array('id'=>$principal_id,'user_type_id'=>'3'), $where_or = array(), $like = array(), $like_or = array(), $order = array(), $start = '', $end = '');
	...
	$this->load->view('template/admin_header');
	$this->load->view('template/admin_leftmenu',$data);
	$this->load->view('manage_principal/principal_details',$data);
	$this->load->view('template/adminfooter_category');
}
...
function delete_data()
{
    ...
    $user_detail= $this->common_model->common($table_name = 'tbl_user', $field = array(), $where = array('user_type_id ='=>'3','id'=>$user_id), $where_or = array(), $like = array(), $like_or = array(), $order = array(), $start = '', $end = '');
    if(@$user_detail[0]->profile_photo!='')
    {
    	unlink("../assets/uploads/principal/".@$user_detail[0]->profile_photo);
    }
...
function change_password_view()
{
   if($this->session->userdata('session_user_id')!='')
	...
	$this->load->view('template/admin_header');
	$this->load->view('template/admin_leftmenu',$data);
	$this->load->view('manage_principal/password_change',$data);
	$this->load->view('template/adminfooter_category');
}
...
{
  $email=$this->input->post('email');
  $check_mail = $this->common_model->common($table_name = 'tbl_user', $field = array(), $where = array('login_email'=>$email,'user_type_id'=>'3'), $where_or = array(), $like = array(), $like_or = array(), $order = array(), $start = '', $end = '');
...
{
  $mobile=$this->input->post('mobile');
  $check_mobile = $this->common_model->common($table_name = 'tbl_user', $field = array(), $where = array('login_mob'=>$mobile,'user_type_id'=>'3'), $where_or = array(), $like = array(), $like_or = array(), $order = array(), $start = '', $end = '');
