...
class manage_head_faculty extends CI_Controller {
...
public function list_view()
{
	...
	$data['head_faculty_list'] = $this->common_model->common($table_name = 'tbl_user', $field = array(), $where = array('user_type_id ='=>'5'), $where_or = array(), $like = array(), $like_or = array(), $order = array('id'=>'DESC'), $start = '', $end = '');
    $data['active']='head_faculty';
	$this->load->view('template/admin_header');
	$this->load->view('template/admin_leftmenu',$data);
	$this->load->view('manage_head_faculty/head_faculty_table',$data);
...
function add_view()
{
	...
	$data['partner_list'] = $this->common_model->common($table_name = 'tbl_user', $field = array(), $where = array('user_type_id'=>'4'), $where_or = array(), $like = array(), $like_or = array(), $order = array(), $start = '', $end = '');
		
    $this->load->view('template/admin_header');
    $this->load->view('template/admin_leftmenu',$data);
    $this->load->view('manage_head_faculty/head_faculty_add',$data);
    $this->load->view('template/adminfooter_category');
}
function insert()
{
		
        $admin_id= $this->session->userdata('session_user_id');
      ...
      $business_name=$this->input->post('business_name');    
    	...
    	if(($_FILES['pro_pic']['name'])!='')
		{
            ...
            {
                move_uploaded_file($file_tmp, "../assets/uploads/head_faculty/" . $new_name . "." . $ext);
                
                $image = $new_name . "." . $ext;
                
                $img_config_4['image_library'] = 'gd2';
                $img_config_4['source_image'] = '../assets/uploads/head_faculty/' . $image;
                $img_config_4['create_thumb'] = FALSE;
                $img_config_4['maintain_ratio'] = FALSE;
                $img_config_4['width']  = 150;
                $img_config_4['height'] = 150;
                $img_config_4['new_image'] ='../assets/uploads/head_faculty/' . $image;
        ...
        $type_id_details=$this->common_model->common($table_name = 'tbl_user', $field = array(), $where = array('user_type_id'=>'5'), $where_or = array(), $like = array(), $like_or = array(), $order = array(), $start = '', $end = '');
        ...
        $final_user_code='HFCT'.$in_id;
    	$data_code = array('user_code' => $final_user_code);
        // print_r($final_user_code); exit;
    	$this->db->where('id', $insert_id);
  		$this->db->update('tbl_user', $data_code);

    	$this->session->set_flashdata('succ_msg','One Head Faculty added successfully');
    	redirect('index.php/manage_head_faculty/list_view/','refresh');
}
...
function edit_view()
{
    ...
    $head_faculty_id =$this->uri->segment(3);
    
    $data['head_fclt_details'] = $this->common_model->common($table_name = 'tbl_user', $field = array(), $where = array('id'=>$head_faculty_id,'user_type_id'=>'5'), $where_or = array(), $like = array(), $like_or = array(), $order = array(), $start = '', $end = '');
    ...
    $country_id= @$data['head_fclt_details'][0]->country;
	...
	$state_id= @$data['head_fclt_details'][0]->state;
    ...
    $this->load->view('template/admin_header');
    $this->load->view('template/admin_leftmenu',$data);
    $this->load->view('manage_head_faculty/head_faculty_edit',$data);
    $this->load->view('template/adminfooter_category');
 }
      ...
      $business_name=$this->input->post('business_name');  
		...
		else
		{
            ...
            if ($ext == "png" || $ext == "jpg" || $ext == "jpeg" || $ext == "gif")
            {
            	if(@$hidden_image!='')
            	{
            		unlink('../assets/uploads/head_faculty/'.@$hidden_image);
            	}
                move_uploaded_file($file_tmp, "../assets/uploads/head_faculty/" . $new_name . "." . $ext);
    	...
    	$this->session->set_flashdata('succ_msg','One Head Faculty updated successfully');
    	redirect('index.php/manage_head_faculty/list_view/','refresh');
}
...
public function details_view()
{
	...
	$head_faculty_id = $this->uri->segment(3);
	$data['user_details'] = $this->common_model->common($table_name = 'tbl_user', $field = array(), $where = array('id'=>$head_faculty_id,'user_type_id'=>'5'), $where_or = array(), $like = array(), $like_or = array(), $order = array(), $start = '', $end = '');

	$partner_id= @$data['user_details'][0]->parent_id;
	$data['partner'] = $this->common_model->common($table_name = 'tbl_user', $field = array(), $where = array('id'=>$partner_id,'user_type_id'=>'4'), $where_or = array(), $like = array(), $like_or = array(), $order = array(), $start = '', $end = '');
	...
	$this->load->view('template/admin_header');
	$this->load->view('template/admin_leftmenu',$data);
	$this->load->view('manage_head_faculty/head_faculty_details',$data);
	$this->load->view('template/adminfooter_category');
}
...
function delete_data()
{
    ...
    $user_detail= $this->common_model->common($table_name = 'tbl_user', $field = array(), $where = array('user_type_id ='=>'5','id'=>$user_id), $where_or = array(), $like = array(), $like_or = array(), $order = array(), $start = '', $end = '');
    if(@$user_detail[0]->profile_photo!='')
    {
    	unlink("../assets/uploads/head_faculty/".@$user_detail[0]->profile_photo);
    }
...
function change_password_view()
{
   if($this->session->userdata('session_user_id')!='')
	...
	$this->load->view('template/admin_header');
	$this->load->view('template/admin_leftmenu',$data);
	$this->load->view('manage_head_faculty/password_change',$data);
	$this->load->view('template/adminfooter_category');
}
...
{
  $email=$this->input->post('email');
  $check_mail = $this->common_model->common($table_name = 'tbl_user', $field = array(), $where = array('login_email'=>$email,'user_type_id'=>'5'), $where_or = array(), $like = array(), $like_or = array(), $order = array(), $start = '', $end = '');
...
{
  $mobile=$this->input->post('mobile');
  $check_mobile = $this->common_model->common($table_name = 'tbl_user', $field = array(), $where = array('login_mob'=>$mobile,'user_type_id'=>'5'), $where_or = array(), $like = array(), $like_or = array(), $order = array(), $start = '', $end = '');
