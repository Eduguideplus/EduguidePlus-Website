...
class manage_mock extends CI_Controller {
...
public function mock_view()
{
	...
	$this->load->view('manage_mock/mock_table_view',$data);
 ...
 function create_slug($string)
  {     
    ...
    function add()
    {
    	...
    	$this->load->view('template/admin_header');
		$this->load->view('template/admin_leftmenu');
		$this->load->view('manage_mock/mock_add_view',$data);
		$this->load->view('template/adminfooter_category');
    }
    ...
    function insert()
    {
    	...
    	$qlist=$this->input->post('qlist');
        $q_id_arr=explode(',',$qlist);
        //print_r($q_id_arr);
    	...
    	$current_date=date('Y-m-d H:i:s');
         ...
         $data_mock= $this->common_model->common($table_name = 'tbl_dummy_mock_set', $field = array(), $where = array(), $where_or = array(), $like = array(), $like_or = array(), $order = array(), $start = '', $end = '');
         if(count($data_mock)>0)
         {
            $this->db->truncate('tbl_dummy_mock_set');
         }

        for($i=0;$i<count($q_id_arr);$i++)
        {
            $data = array(
            ...
            $this->db->insert('tbl_dummy_mock_set',$data);
        }
    	...
    	$this->session->set_flashdata('succ_msg','Dummy Mock Set created successfully');
    	redirect('index.php/manage_mock/mock_view/','refresh');
