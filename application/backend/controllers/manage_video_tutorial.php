...
class manage_video_tutorial extends CI_Controller {
...
public function list_view()
{
	...
	$data['testimonial']=$this->admin_model->selectAll('tbl_video_tutorial');

	$this->load->view('template/admin_header');
	$this->load->view('template/admin_leftmenu');
	$this->load->view('manage_video_tutorial/video_tutorial_table',$data);
 ...
 function create_slug($string)
  {     
    ...
    function add_view()
    {
		...
		$data['category_list'] = $this->admin_model->selectOne('tbl_video_tutorial_category','status','active');
    	...
    	$this->load->view('template/admin_header');
		$this->load->view('template/admin_leftmenu');
		$this->load->view('manage_video_tutorial/video_tutorial_add',$data);
		$this->load->view('template/adminfooter_category');
    }
    function insert()
    {
    	...
    	$data = array(
    					...
    					// 'content'=>$content,
    					'video_slug'=>$this->create_slug($name),
    					'created_by'=>$user_id,
    					'created_on'=>$current_date,
    				);
    	//echo '<pre>'; print_r($data); exit;
    	$this->db->insert('tbl_video_tutorial',$data);
    	$this->session->set_flashdata('succ_msg','added successfully');
    	redirect('index.php/manage_video_tutorial/list_view/','refresh');
...
function delete_data()
{
	...
	/*$image_find = $this->admin_model->selectOne('tbl_video_tutorial','tutorial_id',$id);
	...
	$this->db->delete('tbl_video_tutorial');			
...

{
		...
		$data['test_data'] = $this->admin_model->selectOne('tbl_video_tutorial','tutorial_id',$test_id);
		...
		$this->load->view('template/admin_header');
		$this->load->view('template/admin_leftmenu');
		$this->load->view('manage_video_tutorial/video_tutorial_edit',$data);
		$this->load->view('template/adminfooter_category');
}
function update()
{
    	...
    	$this->db->where('tutorial_id',$test_id);
    	$this->db->update('tbl_video_tutorial',$data);
...
function change_to_active()
{
		...
		for($i=0;$i<count($test_ids);$i++)
		{
			...
			if($this->db->update('tbl_video_tutorial',$data))
			{
				$result=1;
			}
