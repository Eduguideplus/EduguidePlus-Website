...
function list_view()
	{
		   ...
		   $data['active']='manage_event';
	

           $data['tender']= $this->common->select($table_name='tbl_events ',$field=array(),
		  ...
		  $this->load->view('manage_event/list_view',$data);
...
function add_view()
	{
		...
		$data['active']='manage_event';
		  ...
		  $this->load->view('template/admin_header');
		  $this->load->view('template/admin_leftmenu',$data);
		  $this->load->view('manage_event/add');
		  $this->load->view('template/admin_footer');	 
...
function add_tender_submit()
	{
        ...
        else
        {
            ...
            if ($ext == "png" || $ext == "gif" || $ext == "jpg" || $ext == "jpeg" || $ext == "pdf")
            {
                ...
                move_uploaded_file($file_tmp, "../assets/uploads/event/" . $new_name . "." . $ext);
                $image = $new_name . "." . $ext;

                $config['image_library'] = 'gd2';
                $config['source_image'] = '../assets/uploads/event/' . $image;
                ...
                $config['maintain_ratio'] = FALSE;
                $config['new_image'] = '../assets/uploads/event/small/'.$image;
                $config['quality'] = "100%";
                $config['width'] = 271;
                $config['height']= 186;
            ...
            else
            {
                $this->session->set_flashdata('msg','please upload an image or Pdf..!');
                 redirect('index.php/manage_event/list_view','refresh');
            }
...
function edit_tender()
	{
		...
		$data['active']='manage_event';
		  ...
		  $data['tender']=$this->common->select($table_name='tbl_events',$field=array(),$where=array('id'=>$id),$like=array(),$order=array(),$where_or=array(),$like_or_array=array());
		  

		   $this->load->view('template/admin_header');
		   $this->load->view('template/admin_leftmenu',$data);
		   $this->load->view('manage_event/edit',$data);
		   $this->load->view('template/admin_footer');	
	}
        ...
        else
        {
            ...
            if ($ext == "png" || $ext == "gif" || $ext == "jpg" || $ext == "jpeg" || $ext == "pdf")
            {
                ...
                @unlink('../assets/uploads/event/'.@$image);
                 @unlink('../assets/uploads/event/small/'.@$image);
          ...
          $this->db->update('tbl_events',$update_data);
		  $this->session->set_flashdata('succ_update','Successfully updated');
		  redirect('index.php/manage_event/list_view');
	}
...
function delete_tender()
	{
		...
		$desk_details= $this->common->select($table_name='tbl_events',$field=array(),
	...
	function change_to_active()
  {
		...
		for($i=0;$i<count($test_ids);$i++)
		{
			...
			if($this->db->update('tbl_events',$data))
			{
				$result=1;
			}
  ...
  function change_home_status()
{
    ...
    $data=$this->admin_model->selectOne('tbl_events','id',$c_id);
